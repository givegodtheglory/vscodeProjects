...
def get_gaussian_filter_unit_amplitude(time_vector, bt_product, samples_per_symbol):
    ...
    # BT is in the numerator. A smaller BT makes the exponent smaller,
...
def generate_pulse_train(pulse_type, bits, symbol_rate, alpha=None, span=None, BT=None, sampling_freq_hz, 
        ...
        for i, symbol_val in enumerate(bipolar_symbols):
            ...
            if pulse_type == 'Raised Cosine':
                pulse_shape = get_raised_cosine_filter_unit_amplitude(normalized_shifted_time, alpha, samples_per_symbol)
                 ...
                 pulse_shape = get_gaussian_filter_unit_amplitude(normalized_shifted_time, BT, samples_per_symbol)
...
# Inspect pulse trains

#Generate individual pulse and pulse train
bits = np.random(0,2,10)
